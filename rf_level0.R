...
# parameters according to http://localhost:5001/#/experiments/7/runs/4668ed6f53a94e1ba8d254ceec8ebbd0

mtry <- c(130, 162, 43)
trees <- c(1000, 1000, 1000)
min_n <- c(5, 3, 38)
sample.fraction <- c(0.0384246782213566, 0.0319014286128198, 1.0)
...
for(x in 1:nrow(model_parameters)){
  ...
  mlflow_log_param("trees", model_parameters %>% slice(x) %>% pull(trees), run_id = mlflow_run_id, client = tracker)
  mlflow_log_param("min_n", model_parameters %>% slice(x) %>% pull(min_n), run_id = mlflow_run_id, client = tracker)
  mlflow_log_param("sample_fraction", model_parameters %>% slice(x) %>% pull(sample.fraction), run_id = mlflow_run_id, client = tracker)
